...
```
## Описание входных данных для прогнозирования одной категории
- Для прогнозирования по одной категории используется метод `get_prediction`, имеющий следующие параметры:
  * `category_store_id` - идентификатор магазина (категория-магазин)
  * `historical_data` - исторические данные продаж за предыдущих дни. Вид такой:
  ```json    
  [
    {
      "date": "2024-01-01",
    ...
    ```
  * `forecast_days` - число дней, на которые рассчитан прогноз
  - Пример:
  ```json 
     {
         "category_store_id": "FOODS/CA/1",
  ...
  ```
## Описание входных данных для пакетного прогнозирования
- Для прогнозирования всех категорий (пакетное прогнозирование) - метод `get_batch_predictions`, принимающий в качестве параметра
запрос вида:
```json
{
  "requests": [
    {
      "category_store_id": "FOODS/CA/1",
      "historical_data": [
        {"date": "2024-01-01", "sales": 150.5},
        {"date": "2024-01-02", "sales": 180.3}
      ],
      "forecast_days": 7
    },
    {
      ...
      "historical_data": [
        {"date": "2024-01-01", "sales": 80.2},
        {"date": "2024-01-02", "sales": 95.7}
      ],
      "forecast_days": 7
    }
  ]
...
```
- Он состоит по сути из отдельных фрагментов json, который принимал прошлый метод на вход, но каждый 
из этих фрагментов рассчитан на отдельную модель для прогнозирования (разделяются через {}).
## Дополнительно
